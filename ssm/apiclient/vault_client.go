...
func GetVaultClient() (*vault.Client, error) {
	...
	logger.AppLog.Infof("Creating new Vault client for URI: %s", factory.UdmConfig.Configuration.Vault.VaultUri)

	config := vault.DefaultConfig()
	config.Address = factory.UdmConfig.Configuration.Vault.VaultUri

	// Prepare TLS configuration
	tlsConfig := &vault.TLSConfig{
		Insecure: factory.UdmConfig.Configuration.Vault.TLS_Insecure,
	}

	// Handle insecure TLS
	if factory.UdmConfig.Configuration.Vault.TLS_Insecure {
		logger.AppLog.Warnf("TLS_Insecure enabled - skipping certificate verification")
	}

	// Configure mTLS if enabled
	if factory.UdmConfig.Configuration.Vault.MTls != nil {
		logger.AppLog.Infof("Configuring mTLS for Vault client")

		// Verify certificate files exist
		logger.AppLog.Debugf("Loading client certificate from: %s", factory.UdmConfig.Configuration.Vault.MTls.Crt)
		if _, err := os.Stat(factory.UdmConfig.Configuration.Vault.MTls.Crt); err != nil {
		...
		logger.AppLog.Debugf("Loading client key from: %s", factory.UdmConfig.Configuration.Vault.MTls.Key)
		if _, err := os.Stat(factory.UdmConfig.Configuration.Vault.MTls.Key); err != nil {
			logger.AppLog.Errorf("Client key file not found: %v", err)
			return nil, fmt.Errorf("client key file not found: %w", err)
		}

		logger.AppLog.Debugf("Loading CA certificate from: %s", factory.UdmConfig.Configuration.Vault.MTls.Ca)
		if _, err := os.Stat(factory.UdmConfig.Configuration.Vault.MTls.Ca); err != nil {
		...
		tlsConfig.ClientCert = factory.UdmConfig.Configuration.Vault.MTls.Crt
		tlsConfig.ClientKey = factory.UdmConfig.Configuration.Vault.MTls.Key
		tlsConfig.CACert = factory.UdmConfig.Configuration.Vault.MTls.Ca
